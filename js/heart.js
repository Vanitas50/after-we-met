...
/**
 * Living Rose Crystal heart.
 * Built from a parametric heart curve extruded into 3D,
 * wrapped in a custom glass shader with inner light veins,
 * crystal "petal" facets, and a heartbeat pulse.
 */

// ── Heart curve (2D parametric) ───────────────────────────────────────────
...
const heartVertShader = /* glsl */`
  ...
  void main() {
    // Breathing hover: very subtle float
    vec3 pos = position;
...
const galaxyFragShader = /* glsl */`
  ...
  void main() {
    ...
    gl_FragColor = vec4(glow, openAmount * 0.85);
  }
`;

// ── Petal (crystal facet) shader ──────────────────────────────────────────
...
const petalFragShader = /* glsl */`
  ...
  void main() {
    ...
    float alpha = (0.35 + vFresnel * 0.4) * max(0.2, openAmount);
...
export function createHeart(scene) {
  ...
  // Inner galaxy sphere
  const galaxyGeo = new THREE.SphereGeometry(0.55, 32, 32);
  ...
  // Crystal petals (facets around the heart)
  const petalCount = 12;
  const petals = [];
  for (let i = 0; i < petalCount; i++) {
    const angle  = (i / petalCount) * Math.PI * 2;
    const radius = 0.85;
    const pGeo   = new THREE.ConeGeometry(0.18, 0.55, 5);
  ...
  const hintGeo = new THREE.SphereGeometry(1.4, 16, 16);
  ...
  function update(time, delta) {
    // Heartbeat logic: two quick pulses
    if (beatPhase > 0) {
      beatTimer += delta;
      if (beatPhase === 1) {
    ...
    // Hint pulse
    if (hintVisible) {
      hintPulse = (Math.sin(time * 2) + 1) / 2;
      hintMat.opacity = hintPulse * 0.12;
    } else {
      hintMat.opacity *= 0.95;
    }
  }

  return { group, update, triggerHeartbeat, showHint, open, close };
}
